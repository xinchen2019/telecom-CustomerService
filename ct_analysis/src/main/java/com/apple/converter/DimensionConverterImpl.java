package com.apple.converter;

import com.apple.kv.base.BaseDimension;
import com.apple.kv.key.ContactDimension;
import com.apple.kv.key.DateDimension;
import com.apple.utils.JDBCInstance;
import com.apple.utils.JDBCUtil;
import com.apple.utils.LRUCache;
 ...
 * * * * * --Mysql中没有数据 -> 插入该条数据 -> 再次反差该数据，得到id并返回 -> 缓存到内存中
 */
...
public class DimensionConverterImpl implements DimensionConverter {
     ...
     * Logger
     */
    private static Logger logger = LoggerFactory.getLogger(DimensionConverterImpl.class);
     ...
     * 设置SQL语句的具体参数
     *
     * @param preparedStatemen
     * @param dimension
     */
    private void setArguments(PreparedStatement preparedStatemen, BaseDimension dimension) {
        int i = 0;
        try {
            if (dimension instanceof DateDimension) {
                DateDimension dateDimension = (DateDimension) dimension;
                preparedStatemen.setString(++i, dateDimension.getYear());
                preparedStatemen.setString(++i, dateDimension.getMonth());
                preparedStatemen.setString(++i, dateDimension.getDay());
            } else if (dimension instanceof ContactDimension) {
                ContactDimension contactDimension = (ContactDimension) dimension;
                preparedStatemen.setString(++i, contactDimension.getTelephone());
                preparedStatemen.setString(++i, contactDimension.getName());
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private String[] getDateDimensionSQL() {
        String query = "SELECT `id` FROM `tb_dimension_date` WHERE `year`=? AND `year` AND `month`=? AND `day`=? ORDER BY `id`;";
